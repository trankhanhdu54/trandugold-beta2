...
if (isset($_POST['update'])) {
  ...
  header('location:admin-chamngon.php');
      ...
      <div class="content-wrapper">
        <div class="row">
          <div class="col-12 grid-margin stretch-card">
            <div class="card">
              <div class="card-body">
                <h4 class="card-title">Edit Maxim</h4>
                <p class="card-description">
                  Form edit Maxim
                </p>
                <?php
                $update_id = $_GET['update'];
                $show_chamngon = $conn->prepare("SELECT * FROM `chamngon` WHERE id = ?");
                $show_chamngon->execute([$update_id]);
                if ($show_chamngon->rowCount() > 0) {
                  while ($fetch_chamngon = $show_chamngon->fetch(PDO::FETCH_ASSOC)) {
                    ?>
                    <form action="" method="POST" class="forms-sample" enctype="multipart/form-data">
                      <input type="hidden" name="id" value="<?= $fetch_chamngon['id']; ?>">
                      <input type="hidden" name="name" value="<?= $fetch_chamngon['name']; ?>">
                      <input type="hidden" name="old_image" value="<?= $fetch_chamngon['image']; ?>">
                      <img style="width:200px" src="../../../mid_img/<?= $fetch_chamngon['image']; ?>" alt="">
                      <div class="form-group">
                        <label class="card-people mt-auto" for="exampleInputName1">#ID</label>
                        <input type="text" class="form-control" id="exampleInputName1" name="xid" placeholder="Name"
                          value="<?= $fetch_chamngon['id']; ?>">
                      </div>

                      <div class="form-group">
                        <label class="card-people mt-auto" for="exampleInputName1">Name</label>
                        <input type="text" class="form-control" id="exampleInputName1" name="name" placeholder="Name"
                          value="<?= $fetch_chamngon['name']; ?>">
                      </div>
                        ...
                        <label for="file-input" class="drop-container">
                          ...
                          <div class="input-group-append">
                            <input class="file-upload-browse btn btn-primary" type="file" name="image"
                              accept="image/jpg, image/jpeg, image/png, image/webp" id="file-input" class="box">
                          </div>
                        </label>
                      </div>
                      <button type="submit" class="btn btn-success mr-2" name="update">Submit</button>
                      <a href="admin-chamngon.php" class="btn btn-warning">Cancel</a>
                    </form>
                    <?php
                  }
                } else {
